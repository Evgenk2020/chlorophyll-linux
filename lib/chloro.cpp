...
float cl_a_allowance::get_chloro(ch_data dat) const { return static_cast<float>(13.7 * dat.d665 - 5.76 * dat.d649); }
float cl_b_allowance::get_chloro(ch_data dat) const { return static_cast<float>(25.8 * dat.d649 - 7.6 * dat.d665); }
float final_a::get_chloro(ch_data dat) const { return static_cast<float>(dat.vol_filtrate * 0.1 * ((dat.vol_photo_probe + dat.vol_photo_alch) / dat.vol_photo_probe) * _all.get_chloro(dat) / dat.mass_of_probe); }
float final_b::get_chloro(ch_data dat) const { return static_cast<float>(dat.vol_filtrate * 0.1 * ((dat.vol_photo_probe + dat.vol_photo_alch) / dat.vol_photo_probe) * _all.get_chloro(dat) / dat.mass_of_probe); }
